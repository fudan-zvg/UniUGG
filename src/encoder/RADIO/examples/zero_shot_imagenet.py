...
from RADIO.examples.common import collate, round_up, get_standard_transform, get_rank, get_world_size, rank_print, load_model

from mast3r.model_infer import AsymmetricMASt3R
from mast3r.fast_nn import fast_reciprocal_NNs

# import mast3r.utils.path_to_dust3r
from dust3r.inference import inference
from dust3r.utils.image import load_images
from RADIO.utils import get_radio_model_cilp, get_radio_model
...
def main(rank: int = 0, world_size: int = 1):
    ...
    parser.add_argument('-v', '--model-version', default='checkpoints/weight.pth',
    ...
    parser.add_argument('--batch-size', type=int, default=64, #TODO:64
    ...
    model = AsymmetricMASt3R.from_pretrained(args.model_version).to(device)
    model.adaptors = get_radio_model_cilp().to(device).adaptors
    preprocessor = model.radio_model_teacher.make_preprocessor_external().cpu()
    model.eval()

    rank_print('Loading dataset...')
    # ds_builder = load_dataset_builder(args.dataset, trust_remote_code=True, use_auth_token=os.getenv("HUGGINGFACE_TOKEN"))
    ds_builder = load_dataset_builder('data/imagente-1k/val_data_process2', cache_dir="data/imagente-1k")
    ...
    if args.resolution is None:
        ...
        # args.resolution = (model.config.preferred_resolution[0], model.config.preferred_resolution[1])
        args.resolution = (512, 512)
    if args.resize_multiple is None:
        args.resize_multiple = 16
        ...
        for batches in loader:
            for images, targets in batches:
                ...
                with torch.autocast(device.type, dtype=torch.bfloat16, enabled=args.amp):
                    view1 = dict(img=images, true_shape=torch.tensor([384,512]), idx = 0, instance = str(0))
                    output = model.cls_token_adaptors(view1)
                    # output = model.radio_model_teacher(images)
