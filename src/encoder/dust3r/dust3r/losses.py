...
class MultiLoss (nn.Module):
    ...
    def forward(self, *args, **kwargs):
        ...
        if loss_rgb[0] is None:
            return loss, details
        else:
            return loss, loss_rgb, details
...
class ConfLoss (MultiLoss):
    ...
    def __init__(self, pixel_loss, alpha=1, color_loss = False):
        super().__init__()
        assert alpha > 0
        self.alpha = alpha
    ...
    def compute_loss(self, gt1, gt2, pred1, pred2, **kw):
        ...
        if self.color_loss:
            ((loss1, msk1), (loss2, msk2)), (loss_rgb1, loss_rgb2), details = self.pixel_loss(gt1, gt2, pred1, pred2, **kw)
        else:
        ...
        if self.color_loss:
            return conf_loss1 + conf_loss2 + loss_rgb1 + loss_rgb2, dict(conf_loss_1=float(conf_loss1), conf_loss2=float(conf_loss2), loss_rgb1 = float(loss_rgb1), loss_rgb2 = float(loss_rgb2),  **details)
        else:
