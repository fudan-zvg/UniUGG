...
from dust3r.model_dust3rEncoder import AsymmetricCroCo3DStereo, inf  # noqa: F401, needed when loading the model
from dust3r.datasets import get_data_loader  # noqa
from dust3r.losses import *  # noqa: F401, needed when loading the model
from dust3r.inference import loss_of_one_batch, loss_of_one_batch_nature  # noqa
...
def get_args_parser():
    ...
    parser.add_argument('--train_criterion_nature', default="ConfLoss(Regr3D(L21, norm_mode='avg_dis'), alpha=0.2)",
                        type=str, help="train criterion nature")
...
def train(args):
    ...
    train_criterion_nature = eval(args.train_criterion_nature).to(device)
    ...
    model.radio_model_teacher = get_radio_model().to(device).eval()
    model.radio_model_teacher.input_conditioner = nn.Identity() # 不要再二次归一化
    model.encoder = get_radio_model().to(device).train()
    model.encoder.input_conditioner = nn.Identity()
    ...
    for epoch in range(args.start_epoch, args.epochs + 1):
        ...
        train_stats = train_one_epoch(
            model, train_criterion, train_criterion_nature, data_loader_train,
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module, criterion_nature: torch.nn.Module,
    ...
    for data_iter_step, batch in enumerate(metric_logger.log_every(data_loader, args.print_freq, header)):
        ...
        if "Laion" in batch[0]["dataset"] or ("ImageNet" in batch[0]["dataset"]):
            loss_tuple = loss_of_one_batch_nature(batch, model, criterion_nature, device,
