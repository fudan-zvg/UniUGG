...
def geotrf_xyzrgb(Trf, pts, ncol=None, norm=False):
    """
    Apply geometric transformation to xyzrgb points.
    Only xyz coordinates are transformed; rgb remains unchanged.

    Args:
        Trf: Transformation matrix (3x3, 3x4, 4x4 or batched)
        pts: Input points of shape (..., 6), last 3 dims are rgb
        norm: if != 0, project to z=norm plane (useful for projection matrices)

    Returns:
        Transformed points of shape (..., 6)
    """
    assert pts.shape[-1] == 6, "Input must be (..., 6), with [x, y, z, r, g, b]"
    xyz = pts[..., :3]
    rgb = pts[..., 3:]

    xyz_transformed = geotrf(Trf, xyz, ncol=ncol, norm=norm)
    out = np.concatenate([xyz_transformed, rgb], axis=-1) if isinstance(pts, np.ndarray) else torch.cat([xyz_transformed, rgb], dim=-1)
    return out
...
def depthmap_colormap_to_absolute_camera_coordinates(depthmap, camera_intrinsics, camera_pose, img, color_rgb = None, **kw):
    """
    Args:
        - depthmap (HxW array)
        - camera_intrinsics: a 3x3 matrix
        - camera_pose: a 4x3 or 4x4 cam2world matrix
        - color: optional HxWx3 RGB image, will be concatenated to output if provided
    Returns:
        - pointmap of absolute coordinates (HxWx3 or HxWx6 array)
        - mask specifying valid pixels (HxW)
    """
    X_cam, valid_mask = depthmap_to_camera_coordinates(depthmap, camera_intrinsics)
    ...
    assert color.shape[:2] == depthmap.shape, "Color image must match depthmap resolution"
    X_world = np.concatenate([X_world, color], axis=2)  # HxWx6
