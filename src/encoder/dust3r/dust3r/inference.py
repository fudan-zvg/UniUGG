...
def loss_of_one_batch(batch, model, criterion, device, symmetrize_batch=False, use_amp=False, ret=None):
    ...
    with torch.cuda.amp.autocast(enabled=bool(use_amp)):
        ...
        # import numpy as np;np.save("image_l1_ssim.npy", pred1["color_rgb"][0].cpu().numpy())
...
def loss_of_one_batch_nature(batch, model, criterion, device, symmetrize_batch=False, use_amp=False, ret=None):
    view1, view2 = batch
    ignore_keys = set(['depthmap', 'dataset', 'label', 'instance', 'idx', 'true_shape', 'rng'])
    for view in batch:
        for name in view.keys():  # pseudo_focal
    ...
    with torch.cuda.amp.autocast(enabled=bool(use_amp)):
        pred1, pred2 = model(view1, view2, type = "nature")
