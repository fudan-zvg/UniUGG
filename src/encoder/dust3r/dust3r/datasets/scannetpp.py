...
if __name__ == "__main__":
    ...
    dataset = ScanNetpp(split='train', ROOT="/HDD_DISK/data/scannetpp/scannetpp_processed", resolution=224, aug_crop=16)
    print(len(dataset))
    data = dataset[0]
    
    import torch
    from dust3r.utils.geometry import geotrf, geotrf_xyzrgb
    gt_pts1 = geotrf_xyzrgb(torch.from_numpy(data[0]['camera_pose']).unsqueeze(0), torch.from_numpy(data[0]['pts3d']).unsqueeze(0))
    ...
    def normalize_to_clip(rgb_pred):
        ...
        return rgb_pred * std + mean  # 最终变成 clip 的标准归一化结果
    
    import open3d as o3d
    gt_pts1[...,3:6] = normalize_to_clip(gt_pts1[...,3:6])
    gt_pts1 = gt_pts1.reshape(-1,6).cpu().numpy()
    points = gt_pts1[:, :3]
    colors = gt_pts1[:, 3:]
    colors = np.clip(colors, 0, 1)  # 确保颜色在 [0, 1]
