...
def normalize_to_clip(rgb_pred):
    ...
    return (rgb_pred - mean) / std  # 最终变成 clip 的标准归一化结果

def postprocess(out, depth_mode, conf_mode, desc_dim=None, desc_mode='norm', two_confs=False, desc_conf_mode=None, color_rgb = None):
    if desc_conf_mode is None:
        desc_conf_mode = conf_mode
    fmap = out.permute(0, 2, 3, 1)  # B,H,W,D
    res = dict(pts3d=reg_dense_depth(fmap[..., 0:3], mode=depth_mode))
    res["color_rgb"] = color_rgb.permute(0, 2, 3, 1) 
...
class Cat_MLP_LocalFeatures_DPT_Pts3d(PixelwiseTaskWithDPT):
    ...
    def forward(self, decout, img_shape):
        ...
        pts3d, color_rgb = self.dpt(decout, image_size=(img_shape[0], img_shape[1]))
        # color_rgb = normalize_to_clip(color_rgb) # 标准化
