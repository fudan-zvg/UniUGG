...
from dust3r.utils.geometry import depthmap_to_absolute_camera_coordinates, geotrf, depthmap_to_camera_coordinates, depthmap_colormap_to_absolute_camera_coordinates
...
class MASt3RBaseStereoViewDataset(BaseStereoViewDataset):
    ...
    def generate_crops_from_pair(self, view1, view2, resolution, aug_crop_arg, n_crops=4, rng=np.random):
        ...
        if aug_crop_arg is False:
            ...
            for i in range(2):
                ...
                #                                                                             view['camera_intrinsics'],
                #                                                                             view['camera_pose'])
                view['pts3d'], view['valid_mask'] = depthmap_colormap_to_absolute_camera_coordinates(view['depthmap'],
                                                                                            view['camera_intrinsics'],
                                                                                            view['camera_pose'],
                                                                                            view['img'])
            return
        ...
        for i in range(2):
            ...
            #                                                                             view['camera_intrinsics'],
            #                                                                             view['camera_pose'])
            view['color_rgb'] = self.transform(view['img'])
            view['pts3d'], view['valid_mask'] = depthmap_colormap_to_absolute_camera_coordinates(view['depthmap'],
                                                                                        view['camera_intrinsics'],
                                                                                        view['camera_pose'],
                                                                                        view['img'],
                                                                                        view['color_rgb'])
            view.pop('color_rgb', None)
    ...
    def __getitem__(self, idx):
        ...
        for v, view in enumerate(views):
            ...
            view['color_rgb'] = self.transform(view['img'])
            pts3d, valid_mask = depthmap_colormap_to_absolute_camera_coordinates(**view)
            view.pop('color_rgb', None)
...
def transpose_to_landscape(view, revert=False):
    ...
    if width < height:
        ...
        assert view['pts3d'].shape == (height, width, 6)
        view['pts3d'] = view['pts3d'].swapaxes(0, 1)
